...
def _yt_download_to_temp(url: str) -> Optional[str]:
    ...
    # Updated opts for robustness against 403 errors and the 'bool' iterable error
    ydl_opts = {
        # Core settings
        ...
        "quiet": True,
        "no_warnings": True,
        "verbose": False, # Keep deployment logs cleaner
        
        # User-Agent and Cookie fixes for 403 Forbidden
        "cookiefile": False,
        "http_headers": {
        ...
        # Post-processing to ensure a standard MP3 file is created
        "postprocessors": [{
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        # We explicitly request download=True, so 'download' key is available in the info dict
        info = ydl.extract_info(url, download=True)
        
        # --- FIX for 'bool' object is not iterable and reliable path retrieval ---
        # The info dictionary structure is complex. We rely on the internal file path if available.
        path = None
        
        # 1. Check for the definitive path generated by the postprocessor
        # The key 'requested_downloads' might contain the file path. It must be checked for existence first.
        # Ensure 'requested_downloads' exists and is a list/iterable before trying to access elements.
        requested_downloads = info.get('requested_downloads')
        if isinstance(requested_downloads, list) and requested_downloads:
            path = requested_downloads[0].get('filepath')
            
        # 2. If not found, use the fallback filename preparation which is more reliable than direct dict access
        if not path:
             # yt-dlp prepares the filename based on the info dict
            path = ydl.prepare_filename(info)
            # Find the actual downloaded file using glob, since the extension might be different
            import glob
            # We look for files in the temporary directory matching the filename ID
            downloaded_files = glob.glob(os.path.join(tmpdir, f"{info.get('id')}*"))
            # Sort by name (or size, but name is usually fine) and pick the first one
            if downloaded_files:
                path = downloaded_files[0]
            
        if not path or not os.path.exists(path):
            # Raise a specific error if the file was not found locally after successful download attempt
            raise FileNotFoundError(f"Download succeeded, but audio file not found on disk: {url}")
